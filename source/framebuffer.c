...
#include "image.h"
#include "snes.h"
#include "uart.h"                                                                                   // Importing  unistd.h header file
#include "menu.h"
#include "terminal.h"
#include "level2.h"
...
// function to draw image on te screen
...
// function draw pixel on the screen

void drawPixel2(int x, int y, unsigned char attr)
{
    int offs = (y * pitch) + (x * 4);
    *((unsigned int*)(framebuffer + offs)) = vgapal[attr & 0x0f];
}

// function to draw char on the screen
...
void drawChar(unsigned char ch, int x, int y, unsigned char attr)
{
	...
	for (int j=0;j<FONT_WIDTH;j++) {
	    ...
	    drawPixel2(x+j, y+i, col);
	}
	glyph += FONT_BPL;
    }
}

// function to drawString on the screen
